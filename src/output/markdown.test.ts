...
describe("generateMarkdown", () => {
  it("produces header and 0 rules for empty input", () => {
    ...
    expect(md).toContain("0 active rules");
  ...
  it("shows title when present", () => {
    ...
    const md = generateMarkdown(rules, []);
    expect(md).toContain("**Plan Limit**");
  });

  it("shows rationale in italics when present", () => {
    const rules = [makeRule({ rationale: "Prevents abuse" })];
    const md = generateMarkdown(rules, []);
    expect(md).toContain("_Prevents abuse_");
  });

  it("shows owner when present", () => {
    const rules = [makeRule({ owner: "billing-team" })];
    const md = generateMarkdown(rules, []);
    expect(md).toContain("Owner: `billing-team`");
  });

  it("shows since date when present", () => {
    const rules = [makeRule({ since: "2025-06-01" })];
    const md = generateMarkdown(rules, []);
    expect(md).toContain("Since: 2025-06-01");
  });

  it("shows status when present", () => {
    const rules = [makeRule({ status: "active" })];
    const md = generateMarkdown(rules, []);
  ...
  it("shows tags when present", () => {
    ...
    const md = generateMarkdown(rules, []);
    expect(md).toContain("Tags: `billing`, `plans`");
  });

  it("shows links when present", () => {
    const rules = [makeRule({ links: ["https://example.com"] })];
    const md = generateMarkdown(rules, []);
    expect(md).toContain("Links: https://example.com");
  });

  it("shows dependsOn when present", () => {
    const rules = [makeRule({ dependsOn: ["auth-rule"] })];
    const md = generateMarkdown(rules, []);
    expect(md).toContain("Depends on: `auth-rule`");
  });

  it("shows conflictsWith when present", () => {
    const rules = [makeRule({ conflictsWith: ["legacy-rule"] })];
    const md = generateMarkdown(rules, []);
    expect(md).toContain("Conflicts with: `legacy-rule`");
  });

  it("shows supersededBy in active rule detail when status is not historical", () => {
    // A rule with supersededBy but active status is still historical via isHistoricalRule
    // So supersededBy appears in the historical section as SUPERSEDED tag
    const rules = [makeRule({ supersededBy: "new-rule" })];
    const md = generateMarkdown(rules, []);
    expect(md).toContain("[SUPERSEDED by new-rule]");
  });

  it("shows examples when present", () => {
    const rules = [makeRule({ examples: ["Free plan allows 50 items"] })];
    const md = generateMarkdown(rules, []);
    expect(md).toContain("Examples:");
    expect(md).toContain("Free plan allows 50 items");
  });

  it("shows testCases when present", () => {
    const rules = [makeRule({ testCases: ["test/billing.test.ts"] })];
    const md = generateMarkdown(rules, []);
    expect(md).toContain("Tests: `test/billing.test.ts`");
  });

  it("shows historical rules section for deprecated rules", () => {
    const rules = [
      makeRule({ status: "deprecated", description: "old way" }),
      makeRule({ status: "", description: "active rule", scope: "auth", fullScope: "auth" }),
    ];
    const md = generateMarkdown(rules, []);
    expect(md).toContain("## Historical Rules");
    expect(md).toContain("~~old way~~");
    expect(md).toContain("[DEPRECATED]");
  });

  it("shows REMOVED tag for removed rules in historical section", () => {
    const rules = [makeRule({ status: "removed", description: "gone rule" })];
    const md = generateMarkdown(rules, []);
    expect(md).toContain("[REMOVED]");
    expect(md).toContain("~~gone rule~~");
  });

  it("shows SUPERSEDED tag for supersededBy rules in historical section", () => {
  ...
  it("shows active rules count in header for mixed rules", () => {
    const rules = [
      makeRule({ status: "", description: "active" }),
      makeRule({ status: "deprecated", description: "dep" }),
    ];
    const md = generateMarkdown(rules, []);
    expect(md).toContain("1 active rules");
    expect(md).toContain("1 historical");
  });
